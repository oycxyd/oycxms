...
%             datasets = {};
    ...
    if strcmp(options.method,'mz')
        ...
        disp([num2str(ms_references(:,end))])
    else
        [~,I] = sort(lens,'descend');
%         datasets = datasets(I);
        mzs = mzs(I);
...
%% DTW-based alignment
    ...
    mz_recal = cwt2cmz(mzs_recal,'minFreq',ceil(length(mzs)*0.4), ...
    ...
    if strcmp(mode,'workspace')
        ...
        if strcmp(options.method,'mz')
            ...
            [indd1,indd2] = adaptmatch(mzs{1}, mzs_recal{1});
            if strcmp(mode,'recal')
...
%     mz_recal(ms_references(:,n+1))=ms_references(:,1);
    ...
    for n = 1:length(mzs)-1
        ...
        if strcmp(options.method,'mean_spec')
        ...
        if strcmp(options.method,'mz')
            ...
            [indd1,indd2] = adaptmatch(mz2, mzs_recal{n+1});
            if strcmp(mode,'recal')
