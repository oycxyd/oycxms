...
function [list_of_peaks,dims] = pp_mode(varargin)
    filename = varargin{1};
    mode = varargin{2};
    Imin = varargin{3};
...
%         disp(n)
        ...
        % find local maxima for all levels
                ...
                %     [pks,locs] = findpeaks(ridgetable(i,:),'MinPeakHeight',Imin,'MinPeakDistance',3);
                        [pks,locs] = findpeaks(ridgetable(i,:),'MinPeakProminence',Imin,'MinPeakDistance',5);
...
%% peak detection of .imzml files  
    if strcmp(mode, '.imzml')
...
%% peak detection of workspace variables
