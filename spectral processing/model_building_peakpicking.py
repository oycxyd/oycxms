...
def main():
    ...
    X = pd.read_csv("test.csv", delimiter=",",index_col=0)
    print("Input reference file readed")
    ...
    # Combine 3 biological regions
    X_filtered = X_filtered_row

    total_features = len(X_filtered.columns.values)
    ...
    X_filtered = X_filtered.ix[:,clusters]

    print(clusters_features)
    print("Total number features: " + str(len(clusters_features)) + " of " + str(total_features))
    print("Clustering filtering DONE")
    ...
    # For our type of data, it is a good value
    # Can be changed
    stimators = 400
    # Number of parallel jobs
    # We use a cluster for the analysis
    n_jobs = 4

    # As a 1st approach, we allow the RF to grow till the end, as we use a cluster, it is not a problem for us memory or cpu time
    # If a model is good, we reanalyse the data with different parameters to have an idea of the stability of the results
    clf_fs =  RandomForestClassifier(n_estimators=int(stimators), max_depth=None,min_samples_split=1, random_state=0,n_jobs=n_jobs,criterion="gini")

    # Build the RF
    X_scikit_filtered_feature_selected = clf_fs.fit(X_scikit_filtered, y_scikit).transform(X_scikit_filtered)

    print("Model fitted")

    # Creates a matrix with N most important features for the classification
    importances = clf_fs.feature_importances_
    indices = np.argsort(importances)[::-1]
    number_features_selected = X_scikit_filtered_feature_selected.shape[1]
    max_features = 100
    n_feature = 0
    indices_features = []
    for f in range(number_features_selected):
	    if importances[indices[f]]>0 and n_feature<max_features:
		    #feature = list(X_scikit_filtered.columns.values)[indices[f]]
		    indices_features.append(indices[f])
		    n_feature = n_feature + 1
	    if n_feature > max_features:
		    break

    # Calculate mean spectra of selected_features for using as profile in other piece of software
    X_selected_features = X_scikit_filtered.iloc[:,indices_features]
    profile = X_selected_features.groupby(X_selected_features.index).mean()		

    # Calculate Leave-one-sample out cross validation
    loo = LeaveOneOut(number_samples)

    print("Starting LOO CV... will take time...")

    # This step takes long time, depending on the number of jobs, etc.		
    predicted = cross_val_predict(clf_fs, X_scikit_filtered,groups, cv=loo,n_jobs=n_jobs)
    
    print("LOO CV DONE")

    predicted_proba_df = pd.DataFrame(data = predicted, index = samples_names, columns=["Predicted"])
    predicted_proba_df.ix[:,"Actual"] = classes_actual
    predicted_proba_df.to_csv("predicted.csv")
    ...
    # Save model using pickle for using externally
    model = clf_fs
    save_model([model, clusters_features, header, profile],"RFModel.model")
